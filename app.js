...
function actualizarTasa() {
  db.query("CALL get_tasa()", (err, results) => {
    ...
    console.log("Obteniendo tasa de cambio:", tasaActual);
  });
}
...
const seleccionSaboresIndividual = {}; // NUEVO: Maneja el estado de selección de sabores por usuario
...
const hamburguesasCod = {
  ...
  HB2: {
    ...
    precios: { S: 5.5, P: 7.5, C: 9.5 },
  },
  HB3: {
    nombre: "Triple Smash Burger",
    descripcion: "Triple carne",
    precios: { S: 6.5, P: 8.5, C: 10.5 },
  },
  HB4: {
    nombre: "Clásica",
    descripcion:
  ...
  HB9: {
    ...
    descripcion: "TTriple carne",
    precios: { S: 7, P: 9, C: 11 },
  },
  HB10: {
    nombre: "Keto Burger",
    descripcion: "Lechuga, carne, tocienta, ketchup y mayonesa",
    precios: { S: 4.5, P: 6.5, C: 10.5 },
  },
};
...
function getMenuSmashCod() {
  ...
  Object.entries(papasCod).forEach(([cod, data]) => {
    menu += `- *${cod}*: ${data.nombre}  REF.${data.precio}\n`;
  });
  menu += "\n🥤 *Bebidas*\n";
      ...
      case "v":
        if (pedidos[from] && pedidos[from].length > 0) {
          ...
          pedidos[from].forEach((item) => {
            ...
            resumen += `- ${item.cantidad} x ${item.item}${saboresTxt} REF.${item.precio} = REF.${item.subtotal}\n`;
            total += item.subtotal;
          });
      ...
      default:
        ...
        if (matchCodigoArepa) {
          ...
          if (matchCodigoArepa.nombre.includes("2 sabores")) {
            // NUEVO: Si el usuario pide más de 1 arepa, pregunta si quiere sabores diferentes
            if (cantidad > 1) {
        ...
        // --- Lógica para respuesta a pregunta de sabores individuales ---
        if (
          seleccionSaboresIndividual[from] &&
